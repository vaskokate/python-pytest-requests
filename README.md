...
* Проверить ответ метода `GET /pokemons`

Ожидаемый ответ: 
* response `status code` = 200
* в ответе в `json` приходит корректное поле `trainer_name` (для тренера)
* в ответе в `json` приходит корректное поле `name` (для покемона)
...
![image](https://raw.githubusercontent.com/vaskokate/python-pytest-requests/refs/heads/main/python-pytest-requests/pytest-api.jpg)
